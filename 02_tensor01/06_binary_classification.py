...
# 형태
# 평가할 데이터는 정해져 있지 않고(=None), 값은 2개(공부한 시간과 과외한 시간)가 필요함
...
# activation function : sigmoid 함수 사용 (0 또는 1의 값이 나와야 하기 때문에!)
H = tf.sigmoid(logit)
...
# predict와 실제값과 얼마나 같은지
equal = tf.cast(tf.equal(predict, y), dtype=tf.float32)
accuracy = tf.reduce_mean(equal)

# 우리는 지금 데이터의 볼륨이 작아서 학습데이터를 넣어줬지만, 사실 test_set(test_X, test_y)이 필요함
